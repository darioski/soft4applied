...
time_text_left = ax.text(0.52, 0.95, 't={:2.3f}'.format(0), transform=ax.transAxes)
time_text_mean = ax.text(0.01, 0.95, '<x>={:2.3f}'.format(0), transform=ax.transAxes)
time_text_rms = ax.text(0.24, 0.95, 'rms(x)={:2.4f}'.format(0), transform=ax.transAxes)
...
def update(frame):
    ...
    time_text_left.set_text('p(x<0.5)= {:2.3f}'.format(p_left[frame]))
